...
public abstract class Perlin<GradientStorageType> {
	...
	private GradientStorageType[] gradients;
	//Function that performs the dot product (inner product) of two 3D vectors
	//where one of the vectors is stored in the GradientStorageType type.
	private Func<GradientStorageType, double, double, double, double> Dot;

	protected Perlin(GradientStorageType[] gradients, Func<GradientStorageType, double, double, double, double> dot, Func<double, double> smoothingFunction) {
